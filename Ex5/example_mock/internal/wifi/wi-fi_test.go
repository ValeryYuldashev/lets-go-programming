...
type rowTestAddressesSysInfo struct {
	addrs       []string
	errExpected error
}

var testAddressesTable = []rowTestAddressesSysInfo{
	{
...
type rowTestNamesSysInfo struct {
	names       []string
	errExpected error
}

var testNamesTable = []rowTestNamesSysInfo{
	{
		names: []string{"host", "free"},
	},
	{
		errExpected: errors.New("ExpectedError"),
	},
...
func TestGetAddresses(t *testing.T) {
	...
	for i, row := range testAddressesTable {
		mockWifi.On("Interfaces").Unset()
		mockWifi.On("Interfaces").Return(mockAddrIfaces(row.addrs),
			row.errExpected)
...
func TestGetNames(t *testing.T) {
	...
	for i, row := range testNamesTable {
		mockWifi.On("Interfaces").Unset()
		mockWifi.On("Interfaces").Return(mockNamesIfaces(row.names), row.errExpected)
...
func mockNamesIfaces(names []string) []*wifi.Interface {
...
func mockAddrIfaces(addrs []string) []*wifi.Interface {
	var interfaces []*wifi.Interface
